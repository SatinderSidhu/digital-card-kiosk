...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  // Top-level "Overview" is active for /admin, /admin/cards/*, and
  // /admin/reviews/*. "Marketing" is active for anything under
  // /admin/marketing. We don't show a separate back link — the top-level
  // tabs serve as navigation everywhere inside /admin.
  const isMarketing = pathname.startsWith("/admin/marketing");
  const isOverview = !isMarketing;
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between gap-3 px-5 py-3 backdrop-blur-md bg-[color:var(--background)]/80 border-b border-white/5">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <nav className="flex items-center gap-1 ml-2">
            <TabLink href="/admin" active={isOverview}>
              Overview
            </TabLink>
            <TabLink href="/admin/marketing" active={isMarketing}>
              Marketing
            </TabLink>
          </nav>
        </div>
